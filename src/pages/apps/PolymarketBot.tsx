...
export function PolymarketBot({ lang, onLaunch }: { lang: "zh" | "en", onLaunch?: (tab: string) => void }) {
  ...
  const handleConnectWallet = async () => {
    ...
    try {
      // Simulate wallet connection (MetaMask, etc.)
      await new Promise(resolve => setTimeout(resolve, 1500));
      const mockAddress = "0x" + Math.random().toString(16).slice(2, 12) + "..." + Math.random().toString(16).slice(2, 6);
      setWalletConfig(prev => ({ ...prev, walletAddress: mockAddress }));
      alert(lang === 'zh' ? `钱包已连接: ${mockAddress}` : `Wallet connected: ${mockAddress}`);
    } catch (error) {
      console.error(error);
    } finally {
  ...
  const handleExecuteTrade = async (market: any, type: 'YES' | 'NO', amount: number) => {
    try {
      ...
      if (!activeStrat) {
        alert("Please enable at least one strategy first.");
        return;
      }
      ...
      await fetch("/api/polymarket/orders", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          strategy_id: activeStrat.id,
          market_id: market.id || `m-${Math.random()}`,
          market_title: market.title,
          type,
          amount,
          price: type === 'YES' ? market.yes : market.no,
          mode: activeStrat.mode, // Use the strategy's mode
          concurrency: activeStrat.concurrency || 1,
          isServerSide: activeStrat.mode === 'live'
        })
      });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
            ...
            <div className="space-y-4 text-sm text-slate-600 leading-relaxed markdown-body">
              <p>
                {lang === "zh" 
                  ? "AI 正在实时监控市场波动、巨鲸动向及宏观新闻。点击上方按钮生成最新的策略建议。" 
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
              <h2 className="text-lg font-bold text-slate-900 mb-4 flex items-center">
                ...
                {lang === "zh" ? "身份验证与钱包" : "Auth & Wallet"}
              </h2>
              <div className="space-y-4">
                <div className="flex p-1 bg-slate-100 rounded-lg">
                  <button 
                    onClick={() => setAuthMethod("wallet")}
                    className={cn("flex-1 py-1.5 text-xs font-bold rounded-md transition-all", authMethod === "wallet" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500")}
                  >
                    {lang === "zh" ? "钱包授权" : "Wallet Auth"}
                  </button>
                  <button 
                    onClick={() => setAuthMethod("privateKey")}
                    className={cn("flex-1 py-1.5 text-xs font-bold rounded-md transition-all", authMethod === "privateKey" ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500")}
                  >
                ...
                {authMethod === "wallet" ? (
                    ...
                    {walletConfig.walletAddress ? (
                      <div className="p-3 bg-emerald-50 border border-emerald-100 rounded-xl flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-mono text-emerald-700">{walletConfig.walletAddress}</span>
                        </div>
                        <button onClick={() => setWalletConfig(prev => ({ ...prev, walletAddress: "" }))} className="text-[10px] text-slate-400 hover:text-red-500 underline">Disconnect</button>
                      </div>
                    ) : (
                      <button 
                        onClick={handleConnectWallet}
                        disabled={isConnectingWallet}
                        className="w-full py-3 bg-indigo-600 text-white rounded-xl text-sm font-bold hover:bg-indigo-700 transition-all flex items-center justify-center shadow-lg shadow-indigo-200"
                      >
                        {isConnectingWallet ? <RefreshCw className="w-4 h-4 mr-2 animate-spin" /> : <Wallet className="w-4 h-4 mr-2" />}
                  ...
                  <div>
                    <label className="text-sm font-medium text-slate-700 block mb-1.5">
                      {lang === "zh" ? "私钥 / API Key" : "Private Key / API Key"}
                    </label>
                    <div className="flex gap-2">
                      <input 
                        ...
                        placeholder={lang === "zh" ? "输入钱包私钥" : "Enter wallet private key"}
                ...
                <div className="pt-2">
                  <button 
                    ...
                    className="w-full py-2 border border-slate-200 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-50 transition-colors flex items-center justify-center"
                  >
                    {isTestingConnection ? <RefreshCw className="w-4 h-4 mr-2 animate-spin" /> : <Shield className="w-4 h-4 mr-2" />}
                    {lang === "zh" ? "测试连接并验证交易权限" : "Test Connection & Verify Permissions"}
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
              ...
              <div className="space-y-4">
                <div className="p-3 bg-amber-50 border border-amber-100 rounded-xl flex items-start gap-3">
                  ...
                  <div>
                    <div className="text-sm font-bold text-amber-900">{lang === "zh" ? "独享住宅 IP 已激活" : "Dedicated Residential IP Active"}</div>
                    <div className="text-xs text-amber-700 mt-0.5">
                      {lang === "zh" ? "您的账户已绑定独享 IP (103.21.x.x)，每年 $40 维护费已包含在节点费中。" : "Your account is bound to a dedicated IP (103.21.x.x). The $40/year maintenance fee is included in your node fee."}
                    </div>
                  </div>
                </div>
                ...
                {dbStrategies.map((strat) => (
                  <div key={strat.id} className={cn("p-4 rounded-xl border transition-colors", strat.is_active ? "border-indigo-500 bg-indigo-50/30" : "border-slate-200")}>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        <label className="text-[10px] font-bold text-slate-500 block mb-1">{lang === "zh" ? "单笔金额" : "Trade Amount"}</label>
                        <div className="relative">
                          <span className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-400 text-[10px]">$</span>
                          <input 
                            type="number" 
                            value={strat.amount || 10}
                            onChange={(e) => updateStrategy(strat.id, { amount: parseFloat(e.target.value) })}
                            className="w-full pl-5 pr-2 py-1 border border-slate-200 rounded text-xs outline-none font-bold bg-slate-50"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4 mt-4">
                      <div>
                        <label className="text-[10px] font-bold text-slate-500 block mb-1">{lang === "zh" ? "策略权重" : "Strategy Weight"}</label>
                        <input 
                          type="number" 
                          step="0.1"
                          value={strat.weight}
                          onChange={(e) => updateStrategy(strat.id, { weight: parseFloat(e.target.value) })}
